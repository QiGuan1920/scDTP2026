...
def detect_data_scenario(adata):
    ...
    if unique_drugs == 1 and unique_cells > 1:
        return 'mono_drug_multi_cell'
    elif unique_drugs > 1 and unique_cells > 1:
        return 'multi_drug_multi_cell'
    elif unique_drugs > 1 and unique_cells == 1:
        return 'multi_drug_mono_cell'
...
def prepare_data_for_target(adata, drug_attr, drug_sm, drug_img, drug_net, 
    ...
    """Prepare training, validation, and test data for a specific target."""
    set_all_seeds(seed)
    ...
    print(f"Scenario: {scenario_type} | Test target: {target_cell_type} × {target_drug} | Train combinations: {other_combinations}")
    ...
    if len(other_combinations) >= 3:
        ...
        print(f"\nValidation combinations: {valid_combinations}")
    elif len(other_combinations) == 2:
        train_combinations = other_combinations
        valid_combinations = [other_combinations[-1]]
        print(f"⚠️ Only 2 training combinations, using the last one for validation \n Validation combinations: {valid_combinations}")
    else:
    ...
    def get_combination_data(combinations):
        ...
        for cell_type, drug_name in combinations:
            ...
            drug_cells = adata[drug_mask]
            
            if control_cells.n_obs == 0 or drug_cells.n_obs == 0:
                continue
            ...
            drug_features = {
                'attr': drug_attr[drug_name],
                'sm': drug_sm[drug_name],
                'img': drug_img[drug_name],
                'net': drug_net[drug_name]
            }
            ...
            if min_cells > 0:
                ...
                expanded_features = {
                    key: feat.unsqueeze(0).repeat(min_cells, 1)
        ...
        if all_control_expr:
            ...
            combined_features = {
                key: torch.cat([df[key] for df in all_drug_features], dim=0)
                for key in all_drug_features[0].keys()
            }
